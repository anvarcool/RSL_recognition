...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, names, labels, is_train):
        super().__init__()
        self.names = names
        self.labels = labels
        filename = 'train' if is_train else 'test'
        self.data = torch.cat((torch.load(f'data/{filename}.pt'), 
                             torch.load(f'data/{filename}_h2.pt')), dim = 2)
    def __len__(self):
...
class MyDataset_no_augment(torch.utils.data.Dataset):
    def __init__(self, names, labels, is_train):
        ...
        self.labels = labels
        filename = 'train' if is_train else 'test'
        self.data = torch.cat((torch.load(f'data/{filename}_data.pt'), 
                             torch.load(f'data/{filename}_data_h2.pt')), dim = 2)
    def __len__(self):
        return len(self.names)
    def __getitem__(self, index):
